...
res_sm <- terra::rast(file.path(targetdir, "result_SM.tif"))
...
terra::values(res_sm) <- ifelse(
  terra::values(res_sm) < 0.05, NA, terra::values(res_sm)
)
...
tmap_save(tmap_arrange(tm_a, tm_b), filename = "Figure_5.png", device = png,
